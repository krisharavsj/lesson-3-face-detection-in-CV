...
if not cap.isOpened():
    print("ERROR, could not acces webcam")
    exit()
...
def apply_filter(frame,filter_type):
    if filter_type=='GRAYSCALE':
        ...
        sepia_filters=np.array([[0.272, 0.534, 0.131],

[0.349, 0.686, 0.168],

[0.393, 0.769, 0.189]])
        sepia_frame=cv2.transform_frame,sepia_filters()
        sepia_frame=np.clip(sepia_frame,0,255)
        sepia_frame.astype(np.uint8)
...
if not success:
    print("ERROR, failed to read from cam")
    break
img=cv2.flip(img,1)
...
if results.multi_hand_landmarks:
    for hand_landmarks in results.multi_hand_landmarks:
        ...
        thumb_tip=hand_landmarks.landmarks[mp_hands.Hand_Landmark.thumb_tip]
        index_tip=hand_landmarks.landmarks[mp_hands.Hand_Landmark.index_finger_tip]
        middle_tip=hand_landmarks.landmarks[mp_hands.Hand_Landmark.middle_finger_tip]
        ring_tip=hand_landmarks.landmarks[mp_hands.Hand_Landmark.ring_finger_tip]
        pinky_tip=hand_landmarks.landmarks[mp_hands.Hand_Landmark.PINKY_TIP]
        ...
        cv2.circle(img,(ring_x,ring_y),10,(255,0,0),cv2.FILLED)
...
if abs(thumb_x - index_x) < 30 and abs(thumb_y - index_y) < 30:
    ...
    if filters[current_filter] == "GRAYSCALE":
        cv2.imshow("Gesture-Controlled Photo App",
